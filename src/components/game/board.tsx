...
const GameBoard= ({ board, selected, handlePlaceSelected, handleCardClick, invalidCards }:Props) => {
  return (
    <div className="flex justify-center items-center max-h-[100vh] max-w-[100vw]">
      <div className="grid grid-cols-[repeat(11,13rem)] grid-rows-[repeat(11,18rem)] gap-1 border border-gray-300 ">
        {board.map((item,i) => (
          <div key={`card-${i}`} onClick={() => handleCardClick(item)} style={{gridColumn:item.x+1,gridRow:item.y+1}} className={`${invalidCards?.filter(c => c.card===item.card.uid).length ? "relative after:top-0 after:absolute after:z-50 after:w-full after:h-full after:bg-red-600 after:bg-opacity-50":""}`}>
            <Card card={item.card}/>
          </div>
        ))}
